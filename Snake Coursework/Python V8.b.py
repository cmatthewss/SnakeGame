...
mixer.music.load( "C:\\Users\\Audio_files\\title-screen.mid" )  # SET TO CORRECT FOLDER
...
class Snake:
    ...
    def eat_apple(self) -> None:
        ...
        for __apple in self.__apple:
            if self.__x[0] == __apple.x and self.__y[0] == __apple.y:
                ...
                sound1 = mixer.Sound( "C:\\Users\\Audio_files\\sound_effect_1.wav" ) # SOUND EFFECT 
    ...
    def gameover(self) -> bool:
        ...
        if self.__x[0] < 1 or self.__x[0] >= App.BOARD_WIDTH - 1 or self.__y[0] < 1 or self.__y[
            0] >= App.BOARD_HEIGHT - 1:
        ...
        if not self.__snake.gameover:
            ...
            if self.name != "None":
                ...
                self.__canvas.create_text( x, y + App.FONT_DISTANCE * 4, fill=App.COLOR_FONT, font=App.FONT,
                                           text=App.TEXT_TIME_ELAPSED + str(
                                               abs( int( self.total_game_time ) ) ) + "s" )
...
img = PhotoImage(
    file='C:\\Users\\background1.gif' )                #  <--------- BACKGROUND IMAGE
Label( root, image=img ).pack()

leaderboard_button = tk.Button( root, bg='forestgreen', text='Leaderboard', command=leaderboard, width=25,
                                font=('Arial', 48) )
leaderboard_button.pack( side='bottom' )
start_button = tk.Button( root, bg='yellowgreen', text='Single Player', width=25, command=singleplayerwindow,
                          font=('Arial', 48) )
start_button.pack( side='bottom' )


# Namebox= Label(root, bg = 'forestgreen', text='Enter Name Below:', width=25, font= ('Arial', 40))
# Namebox.pack(side='top')
# name = tk.Entry(root, bg = 'yellowgreen', width=25, font= ('Arial', 48))
# name.pack(side='top')
# id = int(time.time())
